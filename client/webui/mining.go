...
func p_miners(w http.ResponseWriter, r *http.Request) {
	...
	config.Lock()
	minerid := config.CFG.Beeps.MinerID
	config.Unlock()

	next_diff_change := 2016-end.Height%2016

	for ; end!=nil; cnt++ {
		if now-int64(end.Timestamp) > int64(config.CFG.MiningStatHours)*3600 {
			break
		}
		...
		miner, mid := config.BlocksMiner(bl)
		if miner!="" {
			om = m[miner]
			om.cnt++
			...
			om.mid = mid
			m[miner] = om
			if current_mid==-1 && minerid==config.MinerIds[mid][1] {
				current_mid = mid
			}
		} else {
			unkn.cnt++
	...
	diff /= float64(cnt)
	bph := float64(cnt)/float64(config.CFG.MiningStatHours)
	hrate := bph/6 * diff * 7158278.826667
	mnrs = strings.Replace(mnrs, "{MINING_HOURS}", fmt.Sprint(config.CFG.MiningStatHours), 1)
	...
	mnrs = strings.Replace(mnrs, "{AVG_DIFFICULTY}", bwlimit.NumberToString(diff), 1)
	mnrs = strings.Replace(mnrs, "{AVG_HASHDATE}", bwlimit.HashrateToString(hrate), 1)
	...
	for i := range srt {
		...
		s = strings.Replace(s, "{MINER_HASHRATE}", bwlimit.HashrateToString(hrate*float64(srt[i].cnt)/float64(cnt)), 1)
	...
	onerow = strings.Replace(onerow, "{MINER_HASHRATE}", bwlimit.HashrateToString(hrate*float64(unkn.cnt)/float64(cnt)), 1)
