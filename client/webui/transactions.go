...
func p_txs(w http.ResponseWriter, r *http.Request) {
	...
	if fil != nil {
		...
		if len(tx2in)>0 {
			wg.Add(1)
			req := &textui.OneUiReq{Param:string(tx2in)}
			req.Done.Add(1)
			req.Handler = func(dat string) {
				txloadresult = textui.LoadRawTx([]byte(dat))
				wg.Done()
			}
	...
	s = strings.Replace(s, "{T2S_SIZE}", bwlimit.BytesToString(sum), 1)
	...
	s = strings.Replace(s, "{TRE_SIZE}", bwlimit.BytesToString(sum), 1)
	...
	if t2s, ok := network.TransactionsToSend[txid.Hash]; ok {
		...
		for i := range tx.TxIn {
			...
			if po != nil {
				...
				fmt.Fprint(w, "<addr>", btc.NewAddrFromPkScript(po.Pk_script, config.AddrVersion).String(), "</addr>")
		...
		for i := range tx.TxOut {
			...
			fmt.Fprint(w, "<addr>", btc.NewAddrFromPkScript(tx.TxOut[i].Pk_script, config.AddrVersion).String(), "</addr>")
			w.Write([]byte("</output>"))
		}
		w.Write([]byte("</outputs>"))
	} else {
...
func raw_tx(w http.ResponseWriter, r *http.Request) {
	...
	if tx, ok := network.TransactionsToSend[txid.Hash]; ok {
		s, _, _, _, _ := textui.DecodeTx(tx.Tx)
		w.Write([]byte(s))
	} else {
