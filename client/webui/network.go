...
func p_net(w http.ResponseWriter, r *http.Request) {
	...
	net_page = strings.Replace(net_page, "{LISTEN_TCP}", fmt.Sprint(config.CFG.Net.ListenTCP, network.TCPServerStarted), 1)
	...
	for idx := range srt {
		...
		s = strings.Replace(s, "{TOTAL_RCVD}", bwlimit.BytesToString(v.BytesReceived), 1)
		s = strings.Replace(s, "{TOTAL_SENT}", bwlimit.BytesToString(v.BytesSent), 1)
		s = strings.Replace(s, "{NODE_VERSION}", fmt.Sprint(v.Node.Version), 1)
		s = strings.Replace(s, "{USER_AGENT}", v.Node.Agent, 1)
		if v.Send.Buf != nil {
			s = strings.Replace(s, "<!--SENDBUF-->", bwlimit.BytesToString(uint64(len(v.Send.Buf))), 1)
		}
