...
func show_info(par string) {
	...
	fmt.Printf("InvsIgn:%d  BlockDups:%d  BlocksNeeded:%d  NetMsgs:%d/%d  UiMsgs:%d  Ticks:%d%s\n", 
		InvsIgnored, BlockDups, BlocksNeeded, len(netBlocks), NetMsgsCnt, UiMsgsCnt, TicksCnt, minago)
