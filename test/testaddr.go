...
func main() {
	...
	for l := 0; l<100; l++ {
		for i := range ta {
			//println(ta[i], "...")
			a, e := btc.NewAddrFromString(ta[i])
			if e != nil {
				println("NewAddrFromString caused error", e.Error())
				return
			}
			
			b24 := append(a.Hash160[:], a.Checksum[:]...)
			a2, e := btc.NewAddrFromDataWithSum(b24[:], a.Version)
			if e != nil {
				println("NewAddrFromDataWithSum caused error", e.Error())
				return
			}
			if a2.Enc58str!=ta[i] {
				println("NewAddrFromDataWithSum failed")
				return
			}
			
			a3 := btc.NewAddrFromHash160(a.Hash160[:], a.Version)
			if a3.Enc58str!=ta[i] {
				println("NewAddrFromHash160 failed")
				return
			}
		}
	}
	...
	fmt.Printf("All tests passed OK and it took %.dms\n", (sto-sta)/1e6)
}
